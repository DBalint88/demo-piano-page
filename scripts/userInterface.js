import {
  createSubmission,
  countCurrentSongAttempts,
  submitSong,
  retractSubmission,
} from "./createSubmission.js";
...
export function hideSongList(displayState, comps) {
  let currentActiveSongList = document.getElementById(
    "level-" + displayState.currentActiveLevel,
  );
...
export function loadSong(e, displayState, comps, userProfile) {
  ...
  displayState.currentSongValue = determineSongValue(
    displayState.currentSongLevel,
...
export function updateButtons(comps, displayState, userProfile) {
  ...
  if (displayState.currentSongFbref == "") {
    ...
    if (userProfile.completedSongs.includes(displayState.currentSongFbref)) {
      ...
      userProfile.pendingSongs.includes(displayState.currentSongFbref)
    ) {
...
export function updateQuotaDisplay(displayState, userProfile, submissionBank) {
  ...
  for (const sub of submissionBank) {
    if (
      sub.userID == userProfile.userID &&
      sub.week == displayState.currentWeek &&
      sub.result == ""
    ) {
