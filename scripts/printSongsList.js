...
export function printSongsList(
  ...
  for (let i = 1; i <= userProfile.viewableSongs.length; i++) {
    ...
    levelValueHeader.textContent =
      "(" + determineSongValue(i, userProfile.handicap) + " points each)";
...
export function checkUserProgress(comps, userProfile, songData, displayState) {
  ...
  let completedSongsPlusPendingSongs = userProfile.completedSongs.concat(
    userProfile.pendingSongs,
  );
  console.log(
    "completedSongsPlusPendingSongs = " + completedSongsPlusPendingSongs,
  );
